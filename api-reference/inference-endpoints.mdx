---
title: "API Endpoints"
description: "Nimbus API endpoints for model registry and analytics. REST API reference for model management, inference, and usage tracking."
...
| `type` | string | Filter by model type: `RxLDA`, `RxGMM`, `RxPolya` (optional) |

<Note>
The model registry is used by the **Julia SDK** and currently includes `RxLDA`, `RxGMM`, and `RxPolya`.  
`RxSTS` / Bayesian STS is **Python SDK only** and does not appear in the model registry.
</Note>
...
- `type` (string): Model type (`RxLDA`, `RxGMM`, or `RxPolya`)
