...
struct BCIMetadata
    ...
    paradigm::Symbol             # :motor_imagery, :p300, :ssvep, :custom
    feature_type::Symbol         # :csp, :bandpower, :erp, :custom
...
struct BatchResult
    ...
    posteriors::Array{Float64, 2} # Full posterior distributions (n_classes × n_trials)
    inference_id::String          # Unique inference identifier
end
```
...
**Parameters:**
- `session::StreamingSession` - Active streaming session
- `method::Symbol` - Aggregation method (`:weighted_vote`, `:majority_vote`)

**Returns:** Final aggregated result for the trial
...
println("Trial prediction: $(final_result.prediction)")
```
