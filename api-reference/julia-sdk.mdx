...
The NimbusSDK.jl Julia package provides production-ready Bayesian inference for Brain-Computer Interface (BCI) applications. Built on **RxInfer.jl**, it offers three models (Bayesian LDA, Bayesian GMM, and RxPolya) with batch and streaming inference capabilities.
...
**Primary Name**: RxPolya (Bayesian Multinomial Probit Regression)  
...
train_model(
    ...
    description::String = "",
    dof_offset::Int = 2,
    mean_prior_precision::Float64 = 0.01,
    predictive_mean_prior::Float64 = 1e6,
    predictive_dof_offset::Int = 2
) -> Model
```
...
- `dof_offset::Int` - Degrees of freedom offset for Wishart priors (default: 2, range: [1, 5])
  - Lower values (1) = less regularization, more data-driven
  - Higher values (3-5) = more regularization, more conservative
- `mean_prior_precision::Float64` - Prior precision for class means (default: 0.01, range: [0.001, 0.1])
  - Lower values = weaker prior, trusts data more
  - Higher values = stronger prior, more regularization
- `predictive_mean_prior::Float64` - Mean prior strength during inference (default: 1e6, range: [1e4, 1e8])
  - Very high values indicate strong trust in learned parameters
- `predictive_dof_offset::Int` - DOF offset for inference (default: 2, range: [1, 5])
  - Should typically match `dof_offset` for consistency

<Note>
**New in v0.2.0**: Hyperparameter configuration allows fine-tuning model behavior for your specific dataset. All hyperparameters are optional with sensible defaults.
</Note>
...
# Train with custom hyperparameters (v0.2.0+)
model = train_model(
    RxLDAModel,
    train_data;
    iterations = 50,
...
<CardGroup cols={2}>
  ...
  <Card title="RxPolya" icon="brain" href="/models/rxpolya">
    Detailed documentation for RxPolya
  </Card>
