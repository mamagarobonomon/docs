---
title: "Julia SDK"
description: "Complete reference for NimbusSDK.jl - Bayesian BCI inference with Bayesian LDA (RxLDA), Bayesian GMM (RxGMM), and Bayesian Softmax (RxPolya). RxInfer.jl-based inference engine."
...
The NimbusSDK.jl Julia package provides production-ready Bayesian inference for Brain-Computer Interface (BCI) applications. Built on **RxInfer.jl**, it offers three models (Bayesian LDA, Bayesian GMM, and Bayesian Softmax) with batch and streaming inference capabilities. **Note**: Bayesian STS is currently available in the Python SDK only.
...
**Primary Name**: Bayesian Softmax (Bayesian Multinomial Logistic Regression)  
**API Name**: `RxPolyaModel`  
**Mathematical Model**: Bayesian Multinomial Logistic Regression

Bayesian multinomial logistic regression (softmax) with continuous transitions. Most flexible for complex multinomial classification tasks.
...
<CardGroup cols={2}>
  ...
  <Card title="Bayesian Softmax (RxPolya)" icon="brain" href="/models/rxpolya">
    Detailed documentation for Bayesian Softmax
  </Card>
