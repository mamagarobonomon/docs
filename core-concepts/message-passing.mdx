...
Bayesian LDA (RxLDA), Bayesian GMM (RxGMM), and Bayesian Softmax (RxPolya) handle temporal structure through proper feature aggregation and trial-level inference.
...
Bayesian LDA (RxLDA), Bayesian GMM (RxGMM), Bayesian Softmax (RxPolya), and Bayesian STS (`NimbusSTS`) all use factor graph representations:
