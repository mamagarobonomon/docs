...
- **Python SDK**: Pre-compiled Bayesian models (LDA, GMM, Softmax, and STS / `NimbusSTS`) with optimized inference engines. The underlying factor graph architecture runs efficiently without requiring manual graph construction
...
NimbusLDA and NimbusQDA (both SDKs), plus the Bayesian Multinomial models (NimbusSoftmax in Python / NimbusProbit in Julia), handle temporal structure through proper feature aggregation and trial-level inference.
...
NimbusLDA and NimbusQDA (both SDKs), Bayesian Multinomial (NimbusSoftmax in Python / NimbusProbit in Julia), and NimbusSTS (Python) all use factor graph representations:
...
**Non-Gaussian Multinomial (NimbusSoftmax / NimbusProbit)**: Multinomial classifier with uncertainty quantification
