...
Both SDKs use Bayesian inference to model uncertainty explicitly with production-ready models: Bayesian LDA, Bayesian GMM, Bayesian Softmax/MPR, and Bayesian STS (Python SDK).
...
<Tip>
The mathematical details are handled automatically by RxInfer. NimbusSDK provides Bayesian LDA (RxLDA), Bayesian GMM (RxGMM), and Bayesian STS (NimbusSTS - Python only) models pre-configured for optimal BCI performance.
</Tip>
