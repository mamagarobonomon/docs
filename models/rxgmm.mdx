---
title: "RxGMM Model"
description: "Gaussian Mixture Model with class-specific covariances using RxInfer.jl"
icon: "brain"
---

# RxGMM - Bayesian Gaussian Mixture Model

RxGMM (Reactive Gaussian Mixture Model) is a Bayesian classification model with **class-specific covariance matrices**, making it more flexible than RxLDA for modeling complex class distributions. Implemented using RxInfer.jl's reactive message passing.

<Note>
**RxGMM is currently implemented** in NimbusSDK.jl and ready for production BCI applications.
</Note>

## Overview

RxGMM extends beyond traditional Gaussian classifiers by allowing each class to have its own covariance structure:
...
- When RxLDA accuracy is unsatisfactory
- When you need maximum flexibility

**Use [RxLDA](/models/rxlda) instead if:**
- Classes are well-separated and have similar spreads
- Speed is critical (RxLDA is faster)
- Training data is limited (RxLDA needs less data)
- Memory is constrained (RxLDA uses less memory)

## Model Architecture

### Mathematical Foundation

RxGMM models class-conditional distributions with **class-specific precision matrices**:
...
**Key Difference from RxLDA**: Each class can have its own covariance structure, making the model more flexible but also more parameter-heavy.
...
<Warning>
**RxGMM requires at least 2 observations per class** to estimate class-specific statistics. Training will fail if any class has fewer than 2 observations.
</Warning>

### Feature Requirements

RxGMM expects **preprocessed features**, not raw EEG:
...
| Paradigm | Classes | Typical Accuracy | When to Use RxGMM |
|----------|---------|------------------|-------------------|
| P300 | 2 (Target/Non-target) | 85-95% | Target/non-target have different variances |
| Motor Imagery | 2-4 | 70-85% | When RxLDA accuracy insufficient |
| SSVEP | 2-6 | 85-98% | Complex frequency responses |

<Note>
RxGMM typically provides 2-5% higher accuracy than RxLDA when class covariances differ significantly, at the cost of ~5-10ms additional latency.
</Note>
...
└─ No → Do classes have similar spreads?
    ├─ Yes → Use RxLDA (faster, same accuracy)
    └─ No → Use RxGMM (more accurate)
```
...
```julia
# RxGMM is ideal for P300 where target/non-target have different variances
...
# If accuracy insufficient, try RxGMM
if rxlda_acc < 0.75
    println("RxLDA accuracy low ($(round(rxlda_acc*100, digits=1))%), trying RxGMM...")
    ...
    println("RxGMM accuracy: $(round(rxgmm_acc*100, digits=1))%")
