...
description: "Bayesian Gaussian Mixture Model with class-specific covariances using RxInfer.jl. Flexible 15-25ms inference for P300 and overlapping class distributions."
keywords: ["Bayesian GMM", "RxGMM", "Gaussian Mixture Model", "P300", "BCI classification", "heteroscedastic", "class-specific covariance", "RxInfer", "NimbusGMM"]
icon: "brain"
---
...
## Comparison: Bayesian GMM vs Bayesian LDA vs Bayesian Softmax vs Bayesian STS

| Aspect | Bayesian GMM (RxGMM) | Bayesian LDA (RxLDA) | Bayesian Softmax (RxPolya) | Bayesian STS (RxSTS) |
|--------|-------|-------|-------|-------|
| **Precision Matrix** | Class-specific | Shared (one for all) | Latent-space precision (K-1) | N/A (state-space) |
| **Mathematical Model** | Heteroscedastic Gaussian Classifier (HGC) | Pooled Gaussian Classifier (PGC) | Multinomial Logistic Regression | State-Space Model (EKF) |
| **Temporal Modeling** | Static (stateless) | Static (stateless) | Static (stateless) | **Dynamic (stateful)** |
| **Training Speed** | Slower | Faster | Moderate | Moderate |
| **Inference Speed** | 15-25ms | 10-15ms | 15-25ms | 20-30ms |
| **Flexibility** | High | Moderate | Highest (non-Gaussian) | Adaptive |
| **Data Requirements** | More | Less | Moderate | Moderate |
| **Memory Usage** | Higher | Lower | Moderate | Moderate (stores state) |
| **Best For** | Heterogeneous class spreads | Homogeneous classes | Complex decision boundaries / non-Gaussian structure | **Non-stationary, long sessions** |
| **Accuracy Gain** | +2-5% (when applicable) | Baseline | Often improves on non-Gaussian tasks | +5-15% (on drifting data) |
...
└─ No → Is speed critical (&lt;15ms)?
    ...
    └─ No → Do classes have different spreads / covariances?
        ├─ Yes → Use Bayesian GMM (more accurate)
        └─ No → Is the decision boundary complex / non-Gaussian?
            ├─ Yes → Use Bayesian Softmax (most flexible)
            └─ No → Use Bayesian LDA (simplest)
```
...
<script
  ...
  dangerouslySetInnerHTML={{
    __html: JSON.stringify({
      ...
      '@type': 'TechArticle',
      headline: 'Bayesian GMM (RxGMM)',
      description:
        'Bayesian Gaussian Mixture Model with class-specific covariances using RxInfer.jl. Flexible 15-25ms inference for P300 and overlapping class distributions.',
      author: {
        '@type': 'Organization',
        name: 'Nimbus BCI',
      ...
      keywords:
        'Bayesian GMM, RxGMM, Gaussian Mixture Model, P300, BCI classification, heteroscedastic, class-specific covariance, RxInfer, NimbusGMM',
      inLanguage: 'en-US',
