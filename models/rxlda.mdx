...
## Comparison: Bayesian LDA vs Bayesian GMM vs Bayesian STS

| Aspect | Bayesian LDA (RxLDA) | Bayesian GMM (RxGMM) | Bayesian STS (RxSTS) |
|--------|-------|-------|-------|
| **Precision Matrix** | Shared across all classes | Class-specific | N/A (state-space) |
| **Mathematical Model** | Pooled Gaussian Classifier (PGC) | Heteroscedastic Gaussian Classifier (HGC) | State-Space Model (EKF) |
| **Temporal Modeling** | Static (stateless) | Static (stateless) | **Dynamic (stateful)** |
| **Training Speed** | Faster | Slower | Moderate |
| **Inference Speed** | Fastest (~10-15ms) | Fast (~15-20ms) | Real-time (~20-30ms) |
| **Flexibility** | Less flexible | More flexible | Adaptive |
| **Best For** | Well-separated classes, stationary data | Overlapping/complex distributions | **Non-stationary data, long sessions** |
| **Memory** | Lower | Higher | Moderate (stores state) |
| **Parameters** | Fewer (n_classes means + 1 precision) | More (n_classes means + precisions) | Features + state dynamics |
...
<script type="application/ld+json">
{
  ...
  "keywords": "Bayesian LDA, RxLDA, Linear Discriminant Analysis, motor imagery, BCI classification, uncertainty quantification, RxInfer, NimbusLDA",
  "inLanguage": "en-US",
