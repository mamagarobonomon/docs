...
description: "Bayesian Linear Discriminant Analysis with uncertainty quantification using RxInfer.jl. Fast 10-15ms inference for motor imagery BCI with shared covariance modeling."
keywords: ["Bayesian LDA", "RxLDA", "Linear Discriminant Analysis", "motor imagery", "BCI classification", "uncertainty quantification", "RxInfer", "NimbusLDA", "pooled Gaussian"]
icon: "brain"
---
...
## Comparison: Bayesian LDA vs Bayesian GMM vs Bayesian Softmax vs Bayesian STS

| Aspect | Bayesian LDA (RxLDA) | Bayesian GMM (RxGMM) | Bayesian Softmax (RxPolya) | Bayesian STS (RxSTS) |
|--------|-------|-------|-------|-------|
| **Precision Matrix** | Shared across all classes | Class-specific | Latent-space precision (K-1) | N/A (state-space) |
| **Mathematical Model** | Pooled Gaussian Classifier (PGC) | Heteroscedastic Gaussian Classifier (HGC) | Multinomial Logistic Regression | State-Space Model (EKF) |
| **Temporal Modeling** | Static (stateless) | Static (stateless) | Static (stateless) | **Dynamic (stateful)** |
| **Training Speed** | Faster | Slower | Moderate | Moderate |
| **Inference Speed** | Fastest (~10-15ms) | Fast (~15-20ms) | Fast (~15-25ms) | Real-time (~20-30ms) |
| **Flexibility** | Lower | Higher | Highest (non-Gaussian) | Adaptive |
| **Best For** | Well-separated classes, stationary data | Overlapping/complex Gaussian structure | Complex multinomial / non-Gaussian boundaries | **Non-stationary data, long sessions** |
| **Memory** | Lower | Higher | Moderate | Moderate (stores state) |
| **Parameters** | Fewer (means + shared precision) | More (means + class precisions) | More (coefficients + latent precision) | Features + state dynamics |

**Decision tree:**
- **Data is stationary** (distributions don't change over time):
  - Well-separated classes → **Bayesian LDA** ✓ Fastest
  - Overlapping classes → **Bayesian GMM** ✓ More flexible
- **Data is stationary but decision boundary is non-Gaussian / complex multinomial**:
  - → **Bayesian Softmax** ✓ Most flexible
- **Data is non-stationary** (drift, fatigue, long sessions):
  - → **Bayesian STS** ✓ Adaptive, handles temporal dynamics
...
<CardGroup cols={2}>
  ...
  <Card title="Bayesian Softmax (RxPolya)" icon="brain" href="/models/rxpolya">
    Most flexible multinomial classifier for complex decision boundaries
  </Card>
...
<script
  ...
  dangerouslySetInnerHTML={{
    __html: JSON.stringify({
      ...
      keywords:
        'Bayesian LDA, RxLDA, Linear Discriminant Analysis, motor imagery, BCI classification, uncertainty quantification, RxInfer, NimbusLDA',
      inLanguage: 'en-US',
