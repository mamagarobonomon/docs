...
# RxPolya - Bayesian Multinomial Probit Regression

**API Name**: `RxPolyaModel`  
**Mathematical Model**: Bayesian Multinomial Probit Regression

RxPolya is a Bayesian multinomial classification model with uncertainty quantification, implemented using RxInfer.jl's reactive message passing framework. It uses **continuous transitions** to map features to a (K-1)-dimensional latent space and **MultinomialPolya likelihood** for probabilistic classification, making it ideal for complex multinomial tasks.

<Note>
**RxPolya is currently implemented** in NimbusSDK.jl and ready for use in production BCI applications. This model provides full Bayesian uncertainty quantification over multinomial distributions, offering advanced capabilities beyond traditional Gaussian classifiers.
</Note>

## Overview

RxPolya implements Bayesian Multinomial Probit Regression, providing a powerful alternative to traditional Gaussian classifiers:
...
- Interpretability of class centers is important (RxPolya is discriminative)
...
RxPolya implements Bayesian Multinomial Probit Regression using continuous transitions and MultinomialPolya likelihood:
...
RxPolya supports configurable hyperparameters for optimal performance tuning:
...
Fine-tune RxPolya for optimal performance on your specific dataset.
...
RxPolya expects **preprocessed features**, not raw EEG:
...
<Note>
RxPolya typically provides comparable or slightly better accuracy than RxLDA/RxGMM for complex distributions, at the cost of ~5-10ms additional latency.
</Note>
...
## Comparison: RxPolya vs RxLDA vs RxGMM

| Aspect | RxPolya | RxLDA | RxGMM |
|--------|---------|-------|-------|
...
```julia
# RxPolya for complex motor imagery with overlapping classes
...
```julia
# RxPolya for P300 target/non-target classification
...
```julia
# Compare RxPolya with other models
