...
description: "Configuring Nimbus for real-time, low-latency BCI applications. Integration with LSL, BrainFlow, and EEG acquisition systems."
...
<Note>
Both SDKs support Bayesian models: **Bayesian LDA**, **Bayesian GMM**, **Bayesian Softmax/MPR**, and **Bayesian STS** (Python SDK only). Choose based on your accuracy, latency, and adaptivity requirements.
</Note>
...
### Bayesian LDA vs Bayesian GMM vs Bayesian STS

<Tabs>
  <Tab title="Python">

```python
from nimbus_bci import NimbusLDA, NimbusGMM, NimbusSTS
...
# Bayesian GMM: More flexible, slightly slower
clf_gmm = NimbusGMM()
...
lda_model = load_model(RxLDAModel, "motor_imagery_4class_v1")
# Typical latency: 10-20ms
# Use when: Speed is priority, classes are well-separated

# RxGMM: More flexible, slightly slower
gmm_model = load_model(RxGMMModel, "motor_imagery_4class_gmm_v1")
# Typical latency: 15-25ms
...
<Tip>
For most real-time applications with **stationary data**, **Bayesian LDA (RxLDA)** is recommended due to its faster inference while maintaining good accuracy. For **long sessions with non-stationary data**, consider **NimbusSTS** (Python SDK).
</Tip>
