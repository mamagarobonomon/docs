...
_Production-ready Bayesian BCI inference with RxLDA, RxGMM, and RxPolya models_

NimbusSDK.jl provides fast, probabilistic inference for Brain-Computer Interface applications. Built on **RxInfer.jl**, the SDK delivers sub-20ms inference latency with full uncertainty quantification through **RxLDA** (Linear Discriminant Analysis), **RxGMM** (Gaussian Mixture Model), and **RxPolya** (Multinomial Probit Regression) implementations.
...
- **Models**: RxLDA (shared covariance), RxGMM (class-specific covariances), RxPolya (multinomial probit regression)
...
- ✅ RxLDA, RxGMM, and RxPolya models
