...
description: "Production-ready Bayesian inference for Brain-Computer Interfaces in Python and Julia. Fast sub-30ms inference latency with NimbusLDA and NimbusQDA, plus NimbusSoftmax/NimbusSTS (Python) and NimbusProbit (Julia). Full uncertainty quantification for motor imagery, P300, and SSVEP paradigms."
...
Nimbus provides fast, probabilistic inference for Brain-Computer Interface applications in **Python** and **Julia**. The SDKs deliver sub-30ms inference latency with full uncertainty quantification through **Bayesian LDA**, **Bayesian QDA**, **Bayesian Softmax**, and **Bayesian STS** implementations.
...
<CardGroup cols={2}>
  ...
  <Card
    title="NimbusSoftmax (Python only)"
    icon="brain"
    href="/models/rxpolya"
  >
    **Bayesian multinomial logistic regression** (Polya-Gamma VI). Flexible non-Gaussian boundaries. Python: `NimbusSoftmax`.
  </Card>
  <Card
    title="NimbusSTS (Python only)"
    icon="brain"
    href="/models/rxsts"
  >
    **State-space model** with EKF-style inference for drift/non-stationarity and online learning. Python: `NimbusSTS`.
  </Card>
  <Card title="NimbusProbit (Julia only)" icon="brain" href="/models/nimbusprobit">
    **Bayesian multinomial probit regression** for non-Gaussian static classification. Julia: `NimbusProbit`.
  </Card>
</CardGroup>
